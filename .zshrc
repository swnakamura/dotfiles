...
# Set background color based on whether on ssh or not
function set_bgcolor() {
    if [[ -z $SSH_CONNECTION ]]; then
        ...
        if [[ $(echo $SSH_CONNECTION | awk '{print $3}') =~ \.(149\.211|148\.167) ]]; then
            # If on the humpback server, set to greenish color
            printf "\x1b]11;#162218\x1b\\"
        else
            # Otherwise, set to reddish color
            printf "\x1b]11;#221816\x1b\\"
        fi
    fi
}
autoload -Uz add-zsh-hook
add-zsh-hook precmd set_bgcolor
set_bgcolor
