...
- Reuse-before-write gate (anti-duplication, evidence-producing)
  - **Trigger:** the moment I am about to write a new `def`/function that performs geometry, projection, coordinate/pose transforms, SE3/Sim3 math, reprojection, or data IO that sounds reusable. This concrete moment — defining the helper — is the gate, not a vague "before adding processing".
  - **Required action before writing the helper:** run and paste into the conversation a repo-wide capability search (`grep -rn` by operation name AND by intent, e.g. `projectPoints`, `T_W_O`, `reproject`, `inv(`), then state explicitly either "no existing implementation found" or "reusing `<path>`". Writing the helper without that pasted evidence is not allowed — "I checked" must be an artifact in the transcript, not a feeling.
  - **Framing discipline:** treat the request as a *capability to achieve* ("reproject mesh into camera"), not a *file to edit* ("modify this viz script"). Narrow file-scoped framing is what silently skips the repo-wide search; reading the one file I happen to be editing is NOT the check.
  - **Preferred permanent fix:** when an existing implementation is found in an ad-hoc script, prefer consolidating it into the shared SSoT package (e.g. `hoi_geometry`) so the capability has exactly one home and the grep check becomes mechanical, rather than copy-reimplementing it.
